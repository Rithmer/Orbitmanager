...
export function normalizeIds(ids?: string | string[] | number[]): number[] {
  if (ids === undefined) {
    return [];
  }
...
export function toRecordById<T extends { id: number }>(
  items: readonly T[],
): Record<number, T> {
...
export function pickGroupedByIds<T>(
...
export function pickRecordByIds<T>(
