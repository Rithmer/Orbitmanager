...
import type { SettingsAppearanceSectionProps } from '@/app/pages/Settings/types'
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <button
              key={option.label}
              onClick={() => {
                if (!option.active) toggleTheme()
              }}
              className={`border-2 rounded-xl p-3 transition-all duration-150 ${
                option.active ? 'border-[#4880ff]' : isDark ? 'border-[#313d4f]' : 'border-gray-200'
              }`}
            >
              <div className={`${option.preview} border rounded-lg p-3 mb-2`}>
                <div className="flex gap-1.5 mb-2">
                  {[60, 40, 50].map((width, index) => (
                    <div key={index} className="h-1.5 bg-gray-300 rounded" style={{ width: `${width}%` }} />
                  ))}
                </div>
                <div className={`h-6 ${option.active ? 'bg-[#4880ff]/20' : 'bg-gray-100'} rounded`} />
              </div>
              <div className="flex items-center justify-center gap-1.5">
                <option.icon className={`w-3.5 h-3.5 ${option.active ? 'text-[#4880ff]' : textSecondary}`} />
                <p className={`text-xs font-semibold ${option.active ? 'text-[#4880ff]' : textSecondary}`}>
                  {option.label}
                </p>
              </div>
            </button>
