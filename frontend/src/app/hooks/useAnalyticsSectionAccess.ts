...
export function useAnalyticsSectionAccess() {
  ...
  return useMemo(() => {
    ...
    const hasLeadOrObserverProjectRole = Object.values(projectMembersByProject)
      .flat()
      .some(
        (m) =>
          m.userId === uid &&
          (m.role === ProjectRole.TEAM_LEAD || m.role === ProjectRole.OBSERVER),
      )

    return {
      allowed: isTeamOwner || hasLeadOrObserverProjectRole,
      isLoading: loading,
    }
  }, [
    enabled,
